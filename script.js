/* Premium entrance loader — waits for the page to finish loading, with a short minimum reveal. */
(function(){
  ...
  const ticker=setInterval(()=>{
    progress=Math.min(98,progress+Math.max(1,Math.round((98-progress)*.12)));
...
document.addEventListener('keydown',e=>{if(e.key==='Escape'){closeModal();closeStepModal();closeContactForm();closeSocialModal();closeWebsiteModal();closeAboutProfile()}});
...
/* Who We Are / founder profile modal */
const aboutProfileModal=document.getElementById('aboutProfileModal');
function openAboutProfile(){if(!aboutProfileModal)return;aboutProfileModal.classList.add('open');aboutProfileModal.setAttribute('aria-hidden','false');document.body.style.overflow='hidden';setTimeout(()=>aboutProfileModal.querySelector('.modal-close')?.focus(),80)}
function closeAboutProfile(){if(!aboutProfileModal)return;aboutProfileModal.classList.remove('open');aboutProfileModal.setAttribute('aria-hidden','true');document.body.style.overflow=''}
document.getElementById('openAboutProfile')?.addEventListener('click',openAboutProfile);
document.getElementById('aboutNav')?.addEventListener('click',e=>{e.preventDefault();document.getElementById('mainNav')?.classList.remove('open');openAboutProfile()});
if(aboutProfileModal){aboutProfileModal.querySelector('.modal-close')?.addEventListener('click',closeAboutProfile);aboutProfileModal.querySelector('.modal-backdrop')?.addEventListener('click',closeAboutProfile)}
...
function openBookingModal(){
  ...
  document.body.style.overflow='hidden';
  setTimeout(()=>bookingModal.querySelector('.modal-close')?.focus(),80);
}
